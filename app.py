...
# =========================
st.set_page_config(
    page_title="AI交易面板 Mobile v13.6",
    layout="wide"
)

st.title("📱 AI交易面板 Mobile v13.6")
st.caption("台股雷達｜盤中通知｜主題掃描")
...
only_volume_alert = st.sidebar.checkbox(
    "推播需成交量放大",
    value=True
)

avoid_hot_rsi = st.sidebar.checkbox(
    "避開 RSI 過熱推播",
    value=True
)
...
def should_alert(judgement):
    ...
    market_time_ok = (
        market_open <= current_minutes <= market_close
    )

    if not (weekday_ok and market_time_ok):
        return False
    ...
    if (
        "強買點" in judgement["action"]
        or "可觀察買點" in judgement["action"]
    ):
        return True
...
st.subheader(
    f"📡 掃描來源：{scan_mode}｜共 {len(stocks)} 檔"
)
...
for i, item in enumerate(stocks):
    ...
    try:
        ...
        change_pct = (
            ((l["Close"] - p["Close"]) / p["Close"]) * 100
        )

        alert_key = (
            f"{datetime.now(TW_TZ).date()}-"
            f"{code}-"
            f"{j['action']}-"
            f"{j['score']}"
        )
        ...
        if should_alert(j):
            ...
            if alert_key not in st.session_state.sent_alerts:
                ...
                buy_price_text = (
                    "不建議追價"
                    if j["buy_price"] is None
                    else f"{j['buy_price']:.2f}"
                )
                ...
                st.session_state.sent_alerts.add(
                    alert_key
                )
...
# =========================
strong_df = df[
    df["分類"].str.contains("今日強勢", na=False)
]

volume_df = df[
    df["分類"].str.contains("爆量股", na=False)
]

macd_df = df[
    df["分類"].str.contains("MACD翻正", na=False)
]

high_df = df[
    df["分類"].str.contains("創波段高", na=False)
]
