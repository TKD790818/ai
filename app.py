...
st.set_page_config(
    page_title="AI交易面板 Mobile v14",
    layout="wide"
)

st.title("📱 AI交易面板 Mobile v14")
st.caption("台股雷達｜Shioaji 即時行情｜盤中爆量｜市場情緒｜主力雷達")
...
for i, item in enumerate(stocks):
    ...
    try:
        ...
        intraday_ratio = calc_intraday_volume_ratio(
            realtime_volume,
            l["VOL20"]
        )
...
c1.metric("市場情緒", sentiment_label, f"分數 {sentiment_score}")
c2.metric("盤中爆量檔數", int((df["盤中量比"] >= intraday_hot_ratio).sum()))
c3.metric("多方強勢檔數", int((df["多方分數"] >= 65).sum()))
c4.metric("空方警戒檔數", int((df["空方分數"] >= 60).sum()))
...
st.dataframe(top5_bear, use_container_width=True)

st.subheader("💥 盤中爆量 Top 5")
hot_volume_df = df.sort_values(by="盤中量比", ascending=False).head(5)
st.dataframe(hot_volume_df, use_container_width=True)
...
st.caption(
    f"最後更新：{datetime.now(TW_TZ).strftime('%Y-%m-%d %H:%M:%S')}｜台灣時間｜Shioaji 即時行情"
)
